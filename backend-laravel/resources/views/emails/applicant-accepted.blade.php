        ...
        <table role="presentation" width="100%" style="max-width:520px;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 8px 24px rgba(0,0,0,0.12);border:1px solid #e5e7eb;">
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  <td align="center" style="vertical-align:middle;">
                    ...
                    <p style="margin:8px 0 0;color:#e8c9a0;font-size:10px;letter-spacing:0.25em;font-weight:bold;text-transform:uppercase;">Application Status Update</p>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding:28px 28px 12px;text-align:left;">
              <h1 style="margin:0 0 12px;font-size:20px;color:#1f2937;">Congratulations, {{ $applicantName }}!</h1>
              <p style="margin:0 0 16px;font-size:14px;color:#4b5563;line-height:1.6;">
                We are pleased to inform you that your application for the position of <strong>{{ $position }}</strong> at Oxford Suites Makati has been <strong>accepted</strong> for the interview stage.
              </p>
              @if(!empty($interviewDate))
              <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:16px;margin:16px 0;">
                <p style="margin:0 0 8px;font-size:13px;font-weight:bold;color:#166534;text-transform:uppercase;letter-spacing:0.05em;">Interview Schedule Details</p>
                <p style="margin:4px 0;font-size:13px;color:#15803d;"><strong>Date:</strong> {{ $interviewDate }}</p>
                @if(!empty($interviewTime))
                <p style="margin:4px 0;font-size:13px;color:#15803d;"><strong>Time:</strong> {{ $interviewTime }}</p>
                @endif
                @if(!empty($interviewMode))
                <p style="margin:4px 0;font-size:13px;color:#15803d;"><strong>Mode:</strong> {{ $interviewMode }}</p>
                @endif
              </div>
              @endif
              <p style="margin:16px 0 0;font-size:14px;color:#4b5563;line-height:1.6;">
                Our HR team will be in touch with any additional guidelines prior to your interview. Please ensure you have a copy of your valid ID and updated resume ready.
              </p>
            </td>
          </tr>
          <tr>
